...
import { Factory } from '../../../core/17-backend-testing/infrastructure/factory';
import { Routes } from '../../../core/17-backend-testing/infrastructure/routes';
...
describe('The Server', () => {
  ...
  it('rejects registration with invalid email', async () => {
    const response = await request(server)
      .post(Routes.register)
      .send({ email: 'test', password: 'TestPass123_' });
  ...
  it('rejects registration with invalid password', async () => {
    const response = await request(server)
      .post(Routes.register)
      .send({ email: 'another@test.es', password: 'test' });
